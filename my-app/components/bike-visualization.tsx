    ...
    <div className="relative h-full flex flex-col landscape:flex-row lg:flex-col overflow-hidden bg-background">
      ...
      <div className="flex flex-col gap-2 flex-none p-2 md:p-0 md:absolute md:top-4 md:left-4 md:bottom-4 md:right-auto z-40 md:pointer-events-none landscape:static landscape:w-1/4 landscape:h-full landscape:overflow-y-auto landscape:border-r landscape:border-border landscape:pointer-events-auto lg:absolute lg:top-4 lg:left-4 lg:bottom-4 lg:w-auto lg:pointer-events-none">
        ...
        <div className="pointer-events-auto bg-card/95 md:bg-background/90 backdrop-blur-md border border-border rounded-xl p-2 shadow-lg w-full md:w-[160px] flex-none">
          <div className="font-bold text-[10px] md:text-xs uppercase tracking-wider hidden md:block text-center text-muted-foreground mb-2">
            Steuerung
          </div>
          
          {/* Action Buttons */}
          <div className="flex flex-row md:flex-col gap-1.5 md:gap-2 flex-wrap justify-center">
            <button onClick={() => { setMeasureMode(!measureMode); setMeasurePoints([]); }} className={`px-2 py-1.5 md:px-3 rounded-md text-xs font-semibold transition-all shadow-sm ${measureMode ? 'bg-[#f39c12] text-white' : 'bg-secondary hover:bg-secondary/80 text-secondary-foreground'}`}>
              {measureMode ? '📏 Aktiv' : '📏 Messen'}
            </button>
            <button onClick={() => setRiderVisible(!riderVisible)} className={`px-2 py-1.5 md:px-3 rounded-md text-xs font-semibold transition-all shadow-sm ${riderVisible ? 'bg-[#22c55e] text-white' : 'bg-secondary hover:bg-secondary/80 text-secondary-foreground'}`}>
              {riderVisible ? '🚴 Fahrer An' : '🚴 Fahrer Aus'}
            </button>
            <button onClick={() => setIsPedaling(!isPedaling)} className={`px-2 py-1.5 md:px-3 rounded-md text-xs font-semibold transition-all shadow-sm ${isPedaling ? 'bg-primary text-primary-foreground' : 'bg-secondary hover:bg-secondary/80 text-secondary-foreground'}`}>
              {isPedaling ? '↻ Animiert' : '↻ Statisch'}
            </button>
            <button onClick={() => setMeasurementsExpanded(!measurementsExpanded)} className="px-2 py-1.5 md:px-3 rounded-md text-xs font-semibold transition-all shadow-sm bg-secondary hover:bg-secondary/80 text-secondary-foreground">
          ...
          {/* Bike Info Legend */}
          {(bikeA || bikeB) && (
            <div className="flex flex-row md:flex-col gap-3 md:gap-2 mt-2 pt-2 border-t border-border justify-center">
              {bikeA && (
                <div className="flex items-center gap-1.5 px-1 md:p-0">
                  <div className="w-3 h-3 rounded-full bg-[#e74c3c] shadow-sm flex-shrink-0" />
                  <span className="font-semibold text-[11px] truncate max-w-[100px] md:max-w-[120px]">{bikeA.brand === '__custom__' ? bikeA.model : `${bikeA.brand} ${bikeA.model}`}</span>
                </div>
              )}
              {bikeB && (
                <div className="flex items-center gap-1.5 px-1 md:p-0">
                  <div className="w-3 h-3 rounded-full bg-[#3498db] shadow-sm flex-shrink-0" />
                  <span className="font-semibold text-[11px] truncate max-w-[100px] md:max-w-[120px]">{bikeB.brand === '__custom__' ? bikeB.model : `${bikeB.brand} ${bikeB.model}`}</span>
                </div>
              )}
            </div>
          )}
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-4 min-h-0 custom-scrollbar">
              <div className="space-y-4">
                ...
                {/* Geometrie Details */}
                <div>
                  <h4 className="text-[11px] font-semibold mb-2 mt-4 text-muted-foreground uppercase tracking-wider">Geometrie Details</h4>
